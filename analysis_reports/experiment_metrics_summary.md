...
| My Method V5 (DINOv2 K=16 target-speed) | IL | C | 79.01 | 53.64 | 0.5215 | N/A | N/A | DINOv2-L/14, K=16 motion tokens, safety v13 finetune, target_speed + route control. |
| My Method V6 (DINOv2 K=16 waypoint-speed) | IL | C | 79.05 | 53.64 | 0.5293* | 233.6209 | 0.3135 | Same V5 checkpoint, eval control uses 1D waypoint-derived speed + route. Best observed DS; `*` ability is offline, without CARLA Traffic Signs junction extra check. |
...
The original metric folders are timestamp-based, while the Bench2Drive tool expects `metric_dir/<save_name>/metric_info.json`. For V2/V3/V4, a temporary symlink index was built under `/tmp` by matching each route `save_name` timestamp to the nearest saved metric timestamp.
